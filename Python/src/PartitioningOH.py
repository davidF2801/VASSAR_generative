...
class PartitioningProblemGAN:
    ...
    def pareto_front_calculator(self, solutions_oh, show, name, save = False, calculate = True):
        ...
        if calculate:
            ...
            solutions = self.translate_one_hot_to_design(solutions_oh)
            ...
            if show or save:
                ...
                if save:
                    ...
                    np.save(os.path.join(path, name + model_name+ '.npy'), np.hstack((sciences, costs, designs)))

                    # Save as CSV
                    np.savetxt(os.path.join(path, name + model_name + '.csv'), np.hstack((sciences, costs, designs)), delimiter=',')
    ...
    def dpp_loss(self, data):
            ...
            p = tf.constant(abs(np.asarray(p)), dtype=tf.float32)
            w = tf.ones(p.shape, dtype=tf.float32)
    ...
    def generate_real_samples(self, num_samples):
        data = []
        training_data_one_hot = np.zeros((0, self.num_instruments*(self.num_instruments+self.num_orbits+1)), dtype=int)  # Initialize as an empty array
        ...
        while len(data) < num_samples:
            ...
            if design_tuple not in data:  # Check if the design is already present in the data list
                ...
                instrument_partitioning_one_hot = tf.one_hot(instrument_partitioning, depth=self.num_instruments)
                orbit_assignment_one_hot = tf.one_hot(orbit_assignment+1, depth=self.num_orbits+1)
                appended = np.append(instrument_partitioning_one_hot, orbit_assignment_one_hot)
                training_data_one_hot = np.vstack((training_data_one_hot, appended))

        # Convert instrument_partitioning and orbit_assignment to one-hot encoding

        #training_data_one_hot = tf.concat([instrument_partitioning_one_hot, orbit_assignment_one_hot], axis=1)

        return training_data_one_hot[:num_samples]  # Return only the required number of samples
    ...
    def generate_fake_samples(self, training):
        ...
        x_softmax = self.generator(noise, training=training)
        ...
        for x in x_softmax:
            ins_diff = x[:self.num_instruments**2].numpy().reshape((self.num_instruments, self.num_instruments))
            orb_dif = x[self.num_instruments**2:].numpy().reshape((self.num_instruments, self.num_orbits+1))
                # Find the maximum value indices in each slice
            ins_max_indices = np.argmax(ins_diff, axis=1)
            orb_max_indices = np.argmax(orb_dif, axis=1)
            
            # Create the output arrays filled with zeros
            ins_output = np.zeros((self.num_instruments, self.num_instruments))
            orb_output = np.zeros((self.num_instruments, self.num_orbits + 1))
            
            # Set the maximum value indices to 1 in each output array
            ins_output[np.arange(self.num_instruments), ins_max_indices] = 1.0
            orb_output[np.arange(self.num_instruments), orb_max_indices] = 1.0
            
            # Flatten the output arrays and concatenate them
            output = np.concatenate([ins_output.flatten(), orb_output.flatten()])
            
            outputs.append(output)

        outputs_tensor = tf.convert_to_tensor(outputs, dtype=tf.float32)
        #outputs_binary = tf.cast(outputs_tensor, dtype=tf.int32)


        return x_softmax,outputs_tensor
    ...
    def train(self):
      # Initialize the GAN training process
            ...
            for batch in real_data_sliced_cons:
                ...
                # Calculate gradients with respect to discriminator trainable variables
                    ...
                    if len(grads) == 0:
                        ...
                        noisy_grads = []
                        for grad in grads:
                            noisy = tf.random.normal(grad.shape, mean=0.0, stddev=1)
                ...
                with tf.GradientTape() as tape:
                    ...
                    generated_samples_oh,generated_samples_thresh_oh=self.generate_fake_samples(training=True)
                    fake_output = self.discriminator(generated_samples_oh, training=False)

                    generated_samples = self.translate_one_hot_to_design(generated_samples_thresh_oh.numpy())


                    feasibility_score = 0
                    for design in generated_samples:
                        design = tuple(design)
                        instruments = design[:12]
                        orbits = design[12:]
                        feasibility_score += self.calculate_feasibility_metric(instruments,orbits)
                    feasibility_score = abs(feasibility_score/len(generated_samples))
        ...
        for nep in range(self.num_episodes):
            ...
            print(f"Episode {nep}")
            #calculate = nep!= 0
            real_data = self.pareto_front_calculator(solutions_oh=data, show=nep==0, save=nep==0, calculate=nep!=0, name='Pareto_Front_INIT')
            ...
            for epoch in range(self.num_epochs):
                ...
                for batch in real_data_sliced:
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        data = np.vstack((data,generated_samples_thresh_oh))
                        # gen_samples_soft = self.gumbel_softmax(generated_samples_thresh, 0.5)
