...
class AssigningProblemGAN:
    ...
    def pareto_front_calculator(self, solutions, show, name, save = False, calculate = True):
        ...
        if calculate:
            ...
            return pareto_designs_array,obj1, obj2
    ...
    def generator_total_loss(self, fake_output, designs_science, designs_cost, designs):
        ...
        return g_loss  + self.gamma1*dpp_loss
    ...
    def dpp_loss(self,data):
            ...
            p = abs(np.asarray(p))
            w = tf.random.uniform(p.shape)

            w = tf.random.uniform(shape=p.shape, minval=0, maxval=1)
    ...
    def generate_real_samples(self,num_samples):
        ...
        for i in range(num_samples):
            ...
            design = np.random.rand(self.num_design_vars)
    ...
    def train(self):
      # Initialize the GAN training process
        ...
        for nep in range(self.num_episodes):
            ...
            print(f"Episode {nep}")
            #calculate = nep!= 0
            real_data = self.pareto_front_calculator(solutions=data, show=nep==0, save=True, calculate=True, name='Pareto_Front_INIT_'+str(nep))
            ...
            for epoch in range(self.num_epochs):
                ...
                for batch in real_data_sliced:
                    ...
                    if len(grads) == 0:
                        ...
                        noisy_grads = []
                        # for grad in grads:
                        #     noisy_grad = grad + tf.random.normal(grad.shape, mean=0.0, stddev=0.1)
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        science = science/len(generated_samples)
                        cost = cost/len(generated_samples)
                        ...
                        data = np.vstack((data,generated_samples))
                ...
                print(f"Epoch: {epoch + 1}/{self.num_epochs}, Discriminator Loss: {d_losses_m}, Generator Loss: {g_losses_m},  Objective 1 =  {science_m}, Objective 2 = {cost_m}")
...
generator = load_model(r'C:\Users\dforn\Documents\TEXASAM\PROJECTS\VASSAR_generative\Results\ContinuousVariables\Models\generator_model_'+model_name+'.h5', custom_objects={'generator_total_loss': GAN.generator_total_loss, 'discriminator_loss':GAN.discriminator_loss, 'genarator_loss':GAN.generator_loss})
# Generate designs
...
for i,design in enumerate(designs):
    ...
    if design in GAN.evaluated_designs:
        obj1_normalized, obj2_normalized = GAN.evaluated_designs[design]
        obj1= obj1_normalized*GAN.science_max
        obj2 = GAN.cost_max*obj2_normalized

    else:
        obj1_normalized, obj2_normalized = GAN.evaluate_design(design)
        obj1=obj1_normalized*GAN.science_max
        obj2 = GAN.cost_max*obj2_normalized
        GAN.evaluated_designs[design] = obj1_normalized, obj2_normalized

    obj1s.append(obj1)
    obj2s.append(obj2)
    # print('Objective 1 = '+str(science))
    # print('Objective 2 = '+str(cost))


_,obj1s_f,obj2s_f = GAN.pareto_front_calculator(designs, show=False, name ='Test5')



path = r'C:\Users\dforn\Documents\TEXASAM\PROJECTS\VASSAR_generative\Results\ContinuousVariables\Pareto_Front\Test5'

# Scatter plot of designs

plt.scatter(obj1s_f, obj2s_f, c='blue', label='Generated Designs')

# Load and plot points from initial_pareto_2.npy
data = np.load(os.path.join(path, 'Pareto_Front_INIT_0.npy'))
initial_obj1 = data[:, 0]  # Extract the costs column
initial_obj2 = data[:, 1]  # Extract the sciences column
plt.scatter(initial_obj1, initial_obj2, c='red', label='Initial Pareto Designs')
...
data = np.hstack((designs, np.array([obj2s, obj1s]).T))
df = pd.DataFrame(data, columns=columns)

# save the DataFrame to an Excel file
output_path = r'C:\Users\dforn\Documents\TEXASAM\PROJECTS\VASSAR_generative\Results\ContinuousVariables\Designs\generated_designs_'+model_name+'.xlsx'
